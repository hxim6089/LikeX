 ...
 *
 * 【设计背景】
 * AI 大模型推理（Ollama qwen3:8b）单次耗时 5-10 秒，
 * 无法满足用户刷新 Feed 时的实时性要求。
 * 参考工业级推荐系统（X、TikTok）的异步预计算架构：
 *
 * 【核心设计：异步预计算 + 内存缓存】
 * - 首次请求（缓存 MISS）：立即降级到传统算法响应（毫秒级），
 *   同时后台异步触发 AI 计算
 * - 后续请求（缓存 HIT）：直接返回缓存的 AI 结果（毫秒级）
 * - 用户行为变化（点赞/点踩）：失效该用户缓存，
 *   下次请求重新触发 AI 计算以反映最新兴趣
 * - TTL 过期（默认 10 分钟）：自动失效，保证推荐新鲜度
 *
 * 【防重复计算】
 * 使用 AtomicBoolean 标记每个用户是否正在计算中，
 * 避免并发请求触发多次重复的 AI 推理。
 *
 * 【线程池】
 * 使用专用线程池 "aiRecExecutor"（2核心/4最大），
 * 隔离 AI 推理的开销，不影响主请求线程。
 */
@Service
