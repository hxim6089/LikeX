...
public class CompareController {
    ...
    private Map<String, Object> calculateCompareStats(
        ...
        Map<String, Object> pDiversity = calculateDiversityStats(pList);
        Map<String, Object> cDiversity = calculateDiversityStats(cList);
        stats.put("personalizedAuthorDedupRate", pDiversity.get("authorDedupRate"));
        stats.put("chronologicalAuthorDedupRate", cDiversity.get("authorDedupRate"));
        stats.put("personalizedUniqueAuthorCount", pDiversity.get("uniqueAuthorCount"));
        stats.put("chronologicalUniqueAuthorCount", cDiversity.get("uniqueAuthorCount"));
        stats.put("personalizedTagCoverageRate", pDiversity.get("tagCoverageRate"));
        stats.put("chronologicalTagCoverageRate", cDiversity.get("tagCoverageRate"));
        stats.put("personalizedUniqueTagCount", pDiversity.get("uniqueTagCount"));
        stats.put("chronologicalUniqueTagCount", cDiversity.get("uniqueTagCount"));
    ...
    private Map<String, Object> calculateDiversityStats(List<ContentWithScore> list) {
        Map<String, Object> stats = new HashMap<>();
        if (list == null || list.isEmpty()) {
            stats.put("authorDedupRate", 0.0);
            stats.put("uniqueAuthorCount", 0);
            stats.put("tagCoverageRate", 0.0);
            stats.put("uniqueTagCount", 0);
            return stats;
        }
        ...
        Set<String> tags = list.stream()
                ...
                .flatMap(content -> {
                    ...
                    if (names.isEmpty() && content.getCategory() != null && !content.getCategory().isBlank()) {
                        names.add(content.getCategory());
                    }
                    return names.stream();
                })
                .collect(Collectors.toCollection(LinkedHashSet::new));

        double authorDedupRate = (double) authorIds.size() / list.size();
        double tagCoverageRate = Math.min(1.0, (double) tags.size() / list.size());
        stats.put("authorDedupRate", round(authorDedupRate));
        stats.put("uniqueAuthorCount", authorIds.size());
        stats.put("tagCoverageRate", round(tagCoverageRate));
        stats.put("uniqueTagCount", tags.size());
        return stats;
    }
