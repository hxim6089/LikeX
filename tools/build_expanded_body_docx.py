...
SOURCE = ROOT / "2959100.2959190.zh.expanded.md"
OUTPUT = Path(r"C:\Users\甄\Downloads\YouTube推荐系统论文翻译_正文扩充15000字.docx")
...
def set_table_width(table, width_cm=14.6):
...
def add_figure_placeholder(doc, line):
    m = re.match(r"\*\*图\s*(\d+)：\*\*\s*(.*)", line)
    if not m:
        return False
    fig_no, caption = m.groups()
    heights = {1: 4.2, 2: 4.0, 3: 6.2, 4: 5.5, 5: 4.2, 6: 5.2, 7: 5.8}
    table = doc.add_table(rows=1, cols=1)
    table.alignment = WD_TABLE_ALIGNMENT.CENTER
    set_table_width(table, 15.0)
    set_table_borders(table, "A6A6A6")
    row = table.rows[0]
    row.height = Cm(heights.get(int(fig_no), 4.5))
...
def build():
    raw = SOURCE.read_text(encoding="utf-8").split("## 7. 参考文献")[0].rstrip()
    ...
    while i < len(lines):
            ...
            if not in_code:
                ...
                for j, code in enumerate(code_lines):
                    if j:
                        p.add_run().add_break()
                    run = p.add_run(code)
                    set_run_font(run, size=10.5, latin="Consolas", east_asia="DengXian")
            i += 1
            continue
        ...
        if is_table_block(lines, i):
            ...
            for cidx, header in enumerate(headers):
                ...
                p = cell.paragraphs[0]
                p.alignment = WD_ALIGN_PARAGRAPH.CENTER
                add_runs(p, header, size=10.5, bold=True)
            for row in rows:
        ...
        if line.startswith("# "):
            ...
            title_seen = True
        elif title_seen and re.match(r"^(Paul|Google|\{)", line):
            p = doc.add_paragraph()
            p.alignment = WD_ALIGN_PARAGRAPH.CENTER
            ...
            p = doc.add_paragraph()
            p.alignment = WD_ALIGN_PARAGRAPH.LEFT if not line.startswith("## 摘要") else WD_ALIGN_PARAGRAPH.CENTER
            p.paragraph_format.first_line_indent = None
            p.paragraph_format.space_before = Pt(10)
            p.paragraph_format.space_after = Pt(4)
            add_runs(p, line[3:], size=14 if not line.startswith("## 摘要") else 12, east_asia=HEAD_EA, bold=True)
        elif line.startswith("### "):
            p = doc.add_paragraph()
            ...
            add_runs(p, line[4:], size=12, east_asia=HEAD_EA, bold=True)
        elif line.startswith("#### "):
            p = doc.add_paragraph()
            p.alignment = WD_ALIGN_PARAGRAPH.LEFT
            p.paragraph_format.first_line_indent = None
            ...
            p.paragraph_format.space_after = Pt(3)
            add_runs(p, line[5:], size=12, east_asia=HEAD_EA, bold=True)
        elif line.startswith("- "):
