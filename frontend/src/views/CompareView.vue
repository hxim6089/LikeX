  ...
  <div class="compare-view">
    ...
    <div class="section">
      ...
      <div class="stats-bar" v-if="stats">
        ...
        <div class="stat-card">
          <div class="stat-value">{{ Math.round((stats.personalizedAuthorDedupRate || 0) * 100) }}%</div>
          <div class="stat-label">作者去重率</div>
          <div class="stat-sub">{{ stats.personalizedUniqueAuthorCount || 0 }} 位作者 vs {{ stats.chronologicalUniqueAuthorCount || 0 }}</div>
        </div>
        <div class="stat-card">
          <div class="stat-value">{{ Math.round((stats.personalizedTagCoverageRate || 0) * 100) }}%</div>
          <div class="stat-label">标签覆盖率</div>
          <div class="stat-sub">{{ stats.personalizedUniqueTagCount || 0 }} 个标签 vs {{ stats.chronologicalUniqueTagCount || 0 }}</div>
        </div>
      </div>
...
const renderBarChart = () => {
  ...
  barChartInstance.setOption({
    ...
    xAxis: { type: 'category', data: ['平均分', '标签命中率(%)', '互动量', 'TF-IDF 相似度', '作者去重率(%)', '标签覆盖率(%)'] },
    yAxis: { type: 'value' },
    series: [
      {
        ...
        data: [
          ...
          Math.round((s.personalizedAuthorDedupRate || 0) * 100),
          Math.round((s.personalizedTagCoverageRate || 0) * 100)
        ]
      },
      {
        name: '时间倒序', type: 'bar',
        ...
        data: [
          ...
          Math.round((s.chronologicalAuthorDedupRate || 0) * 100),
          Math.round((s.chronologicalTagCoverageRate || 0) * 100)
        ]
      }
    ]
  })
