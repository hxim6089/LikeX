  ...
  <div class="feed-wrapper">
    ...
    <template v-for="(item, index) in items" :key="item.id || item.content?.id">
        <TweetCard 
            ...
            @hide-content="handleContentHidden"
            @hide-author="handleAuthorHidden"
        />
        <!-- 按配置频率插入广告 -->
        <AdCard 
...
const handleContentHidden = (contentId) => {
    items.value = items.value.filter(item => (item.content?.id || item.id) !== contentId);
}

const handleAuthorHidden = (authorId) => {
    items.value = items.value.filter(item => {
        const content = item.content || item;
        return content.author?.id !== authorId;
    });
}
