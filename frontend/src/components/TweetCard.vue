<template>
  <div v-if="!isHidden" class="tweet-card" ref="tweetCardRef" @click="goToDetail">
    ...
    <div class="tweet-content">
      <div class="tweet-header">
        ...
        <div class="header-actions">
             <el-dropdown
               v-if="tweet.author?.id !== currentUser.id"
               trigger="click"
               @command="handleFeedbackCommand"
             >
               <el-icon class="more-btn" @click.stop title="更多"><MoreFilled /></el-icon>
               <template #dropdown>
                 <el-dropdown-menu>
                   <el-dropdown-item command="notInterested">对此内容不感兴趣</el-dropdown-item>
                   <el-dropdown-item command="mute">静音该作者</el-dropdown-item>
                   <el-dropdown-item command="block" divided>屏蔽该作者</el-dropdown-item>
                 </el-dropdown-menu>
               </template>
             </el-dropdown>
...
<script setup>
import { ChatLineRound, Star, StarFilled, Refresh, DataAnalysis, Cpu, Delete, SortDown, MoreFilled } from '@element-plus/icons-vue'
...
const emit = defineEmits(['deleted', 'hide-content', 'hide-author'])
...
const startViewTracking = () => {
  if (viewStartTime || hasReportedView || hasReportedSkip || !props.tweet?.id) return
...
const stopViewTracking = () => {
  ...
  } else if (durationMs >= 300 && !hasMeaningfulInteraction && currentUser.id) {
    hasReportedSkip = true
    api.post('/behavior/skip', {
      userId: currentUser.id,
      contentId: props.tweet.id
    }).catch(() => {})
  }
...
const markMeaningfulInteraction = () => {
...
const handleFeedbackCommand = async (command) => {
    if (!currentUser.id || !props.tweet?.id || !props.tweet?.author?.id) return
    markMeaningfulInteraction()

    if (command === 'notInterested') {
        await api.post('/signal/not-interested', {
            userId: currentUser.id,
            contentId: props.tweet.id
        })
        isHidden.value = true
        emit('hide-content', props.tweet.id)
        ElMessage.success('已减少此类内容推荐')
        return
    }

    await api.post(`/signal/${command}`, {
        userId: currentUser.id,
        authorId: props.tweet.author.id
    })
    isHidden.value = true
    emit('hide-author', props.tweet.author.id)
    ElMessage.success(command === 'mute' ? '已静音该作者' : '已屏蔽该作者')
}
